        ...
        docsArray.forEach((docSnap) => {
            ...
            if (audioEl) {
                ...
                audioEl.addEventListener("error", () => {
                    ...
                    if (info && !info.querySelector(".media-broken-audio")) {
                        const warn = document.createElement("p");
                        warn.className = "media-broken-audio";
                        warn.textContent = "⚠️ Fichier audio introuvable : " + (docSnap.data().audioUrl || "URL vide");
                        info.insertBefore(warn, audioEl);
                    }
                }, { once: true });

            }

            const imgEl = card.querySelector(".beat-card-media img");
            if (imgEl) {

                imgEl.addEventListener("error", () => {
                    const media = card.querySelector(".beat-card-media");
                    if (media) {
                        media.innerHTML =
                            '<div class="media-broken">' +
                                '🖼️ Image introuvable<br>' +
                                '<small>' + escapeHtml(docSnap.data().imageUrl || "URL vide") + '</small>' +
                            '</div>';
                    }
                }, { once: true });
